...
// Define interface for raw social media data from JSON
interface RawSocialMediaData {
    name: string;
    url: string;
...
const SocialMediaGrid = () => {
    ...
    useEffect(() => {
        // Fetch social media data from the public data file
        fetch('/data.json')
            .then(response => response.json())
            .then(data => {
                // Map the data to include the icons
                const socialLinks = data.socialMedia.map((link: RawSocialMediaData) => ({
                    ...link,
            ...
            <div className="grid grid-cols-3 gap-4">
                {socialMediaLinks.map((link) => (
                    <a
                        ...
                        className={`flex flex-col items-center p-4 bg-gradient-to-br ${link.gradientClasses} rounded-xl text-black hover:scale-105 transition-transform`}
                    >
                        {link.icon}
                    </a>
                ))}
